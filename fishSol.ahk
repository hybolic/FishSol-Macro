...
;IF YOU USE <Visual Studio Code> or Other Similar IDE Software this section should be
;collaspeable using the down arrow over there.  -Nadir
;<== there will be similar areas throught the entire codebase to help with navigation
    ; if it works and the arrow exists
    ; this area should collapse
;end
...
;path to the script icon
iconFilePath := A_ScriptDir "\img\icon.ico"
;if the icon exits we set it as the icon
;   in the tray/windowbar
...
;<== default variables
 ...
 ;if the ini file exists we grab variables for plugins
...
; another check for if ini/settings file exits and if it does we
; override the default values with the users
;<== load saved values from the ini/settings file
 if (FileExist(iniFilePath)) {
...
;<== if <plugin>.ahk exits we set the has<name>Plugin to true
...
;crafter plugin variable ini/settings check
 if (FileExist(iniFilePath)) {
     IniRead, tempCrafter, %iniFilePath%, "Macro", "crafterToggle"
     if (tempCrafter != "ERROR")
     {
...
;<==this extracts the private server code directly from the url
 code := ""
...
;<== Randomize Stuff
 /* <==
   Okay this section is fairly complex looking so instead of 
   fully labeling everything i will summarize the goal!
   -----------------------------------------------------------------------------
   we are choosing 2 random numbers one between 1 and 6, the other from 1 to 10.
   using number two<messageRand> we select a random message from the first if else statement.
   using number one<Shuffle> we use it to select a predefined random order of devs to place in
   the credits section, then using the name set in the variable we grab the picture from file.
 */
 Random,, A_TickCount
 Random, shuffle, 1, 6
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;ANYTHING FROM HERE TO THE NEXT MARKER IS BASICALLY ALL GUI STUFF AND VERY MUCH OUT OF MY EXPERTISE;
;WHEN IT COMES TO SIMPLE DOCUMENTATION, SORRY EVERYONE - NADIR  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;<===;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

    ;<===Main window setup
     Gui, Color, 0x1E1E1E
     Gui, Font, s17 cWhite Bold, Segoe UI
     Gui, Add, Text, x0 y10 w600 h45 Center BackgroundTrans c0x00D4FF, fishSol v1.9.4
    ...
    ;<== Credits Tab
     ...
     Gui, Font, s8 c0xCCCCCC Normal
     Gui, Add, Text, x50 y518 w500 h15 BackgroundTrans, fishSol v1.9.4 - %randomMessage%
 
     Gui, Show, w600 h670, fishSol v1.9.4
...
; ends the <biome.ahk> background process
;<== when closing out of <fishSol.ahk>
 GuiClose:
    if (biomeDetectionRunning) {
        ...
        biomeDetectionRunning := false
    } ;lastly send the webhook telling the server that the macro
      ;   has stopped
...
;<== variable references
 toggle := false    ; [DEV NOTE] because these are outside the startup
 firstLoop := true  ; routine they aren't actually set and are actually
 startTick := 0     ; blank when first called ie -> "" or NULL
 cycleCount := 0    ; 
;end
...
;<==INI SETTINGS PART1
 ; toggle and save the Loops to the ini/settings file
 ;         both max Loop and fishing Loop
 UpdateLoopCount:
     Gui, Submit, nohide
     if (MaxLoopInput > 0) {
 ...
 ; toggle and save the Sell mode to the ini/settings file
 ToggleSellAll:
 ...
 ; toggle and save the fishing detection mode to the ini/settings file
 ...
 ; toggle and save the Azerty switch to the ini/settings file
 ;          this just switches qwerty layout azerty
 ToggleAzertyPathing:
 ...
 ; toggle and save the Auto Unequip Aura to the ini/settings file
 ToggleAutoUnequip:
 ...
 ; toggle and save the Auto close chat to the ini/settings file
 ToggleAutoCloseChat:
 ...
 ; toggle and save the Strange Controller Use to the ini/settings file
 ...
 ; toggle and save the Biome Randomizer use to the ini/settings file
 ...
 ; toggle and save the current failsafe webhook sending to the ini/settings file
 ...
 ; toggle and save the current pathing failure to the ini/settings file
 ...
 ; toggle and save the current item failure webhook to the ini/settings file
 ToggleItemWebhook:
 ...
 ; toggle and save the current using autocrafter to the ini/settings file
 ToggleCrafter:
 ...
 ; completely toggle and save the current snowman pathing to the ini/settings file
 ;      and save autocrafter/webhook to ini/settings file?
 ...
 ; toggle and save the current snowman pathing failure webhook to the ini/settings file
 ...
 ; toggle and save the current autocrafter status time to the ini/settings file
 ToggleAutoCrafter:
 ...
 ; update and save the current autocrafter time to the ini/settings file
 ...
 ; toggle and save the current autocrafter failure webhook? to the ini/settings file
 ; [DEV COMMENT] TODO: FIGURE OUT WHAT THIS ACTUALLY DOES
...
;<==Ini Saving Part2
 ; saves the current private server link to the ini/settings file
 UpdatePrivateServer:
     Gui, Submit, nohide
 ...
 ; saves the current fishing failsafe time to the ini/settings file
 ...
 ; saves the current pathfinding failsafe time to the ini/settings file
 ...
 ; saves the current rejoin wait to the ini/settings file
 ...
 ; saves the current pixel amount for advanced fishing to the ini/settings file
 ...
 ; saves the current discord webhook url to the ini/settings file
 UpdateWebhook:
     Gui, Submit, nohide
 ...
 ; saves the current biomes private server link to the ini/settings file
 ...
 ; saves the current Biome Toggles to the ini/settings file
 SaveBiomeToggles:
     Gui, Submit, NoHide
...
SendWebhook(title, color := "16777215") {
    ...
    . "    ""title"": """ title ""","
    . "    ""color"": " color ","
    . "    ""footer"": {""text"": ""fishSol v1.9.4"", ""icon_url"": ""https://maxstellar.github.io/fishSol%20icon.png""},"
    . "    ""timestamp"": """ timestamp """"
    . "  }"
    . "],"
    . """content"": """""
...
RunStrangeController() {
    ...
    if (res = "1080p") {
        sleep 300
        MouseMove, 46, 520, 3   ;Inventory Button
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 1279, 342, 3 ;Items Tab
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 1104, 368, 3 ;Search Bar
        sleep 300
        MouseClick, Left
        ...
        Send, ^v
        sleep 300
        MouseMove, 848, 479, 3 ;First Item slot
        sleep 300
        MouseClick, Left
        sleep 300
        ...
        MouseMove, 682, 578, 3  ;Use item Button
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 1413, 297, 3 ;Close Window
        sleep 300
        MouseClick, Left
        sleep 300
    }
    ; 1440p
        ...
        sleep 300
        MouseMove, 52, 693, 3   ;Inventory Button
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 1704, 452, 3 ;Items Tab
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 1473, 489, 3 ;Search Bar
        sleep 300
        MouseClick, Left
        Clipboard := "Strange Controller"
        sleep 300
        Send, ^v
        sleep 300
        MouseMove, 1144, 643, 3 ;First Item slot
        sleep 300
        MouseClick, Left
        sleep 300
        ...
        MouseMove, 920, 774, 3  ;Use item Button
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 1896, 403, 3 ;Close Window
        sleep 300
        MouseClick, Left
        sleep 300
    }
    ; 1366x768
        ...
        sleep 300
        MouseMove, 42, 376, 3   ;Inventory Button
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 911, 242, 3 ;Items Tab
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 785, 262, 3 ;Search Bar
        sleep 300
        MouseClick, Left
        Clipboard := "Strange Controller"
        sleep 300
        Send, ^v
        sleep 300
        MouseMove, 616, 347, 3 ;First Item slot
        sleep 300
        MouseClick, Left
        sleep 300
        ...
        MouseMove, 486, 413, 3  ;Use item Button
        sleep 300
        MouseClick, Left
        sleep 300
        MouseMove, 1017, 214, 3 ;Close Window
        sleep 300
        MouseClick, Left
        sleep 300
    }
        ...
        Loop {
            ...
            if (snowmanPathing) {
                ...
                if ((snowmanPathingLastRun = 0 && elapsed >= snowmanPathingTime) || (snowmanPathingLastRun > 0 && (elapsed - snowmanPathingLastRun) >= snowmanPathingInterval)) {
                    Send, {Esc}     ; <---- Reset player position
                    Sleep, 650      ;  |
                    Send, R         ;  |
                    Sleep, 650      ;  |
                    Send, {Enter}   ;  |
            ...
            if (hasCrafterPlugin && crafterToggle && autoCrafterDetection) {
                ...
                if (currentTime - autoCrafterLastCheck >= autoCrafterCheckInterval) {
                    ...
                    ;Search possition for pixel of color if ErrorLevel 0 do RunAutoCrafter()
            ...
            if (loopCount > maxLoopCount || restartPathing) {
                ...
                ;wait for snowmanPathing to be done
                if (snowmanPathing) {
                    Sleep, 2000
                }
                
                ;check if we have a webhook and pathingWebhook is true try to post message
                if (pathingWebhook) {
                    try SendWebhook(":moneybag: Starting Auto-Sell Pathing...", "16636040")
                }

                if (autoUnequip) {
                    MouseMove, 45, 412, 3 ; Auras Inventory
                    sleep 300
                    Click, Left
                    sleep 300
                    MouseMove, 830, 441, 3 ; Slot One
                    sleep 300
                    Click, Left
                    sleep 300
                    MouseMove, 634, 638, 3 ; Unequip/Equip button
                    sleep 300
                    Click, Left
                    sleep 1200
                    Click, Left
                    sleep 300
                    MouseMove, 1425, 303, 3 ; Close Gui Button
                    sleep 300
                    Click, Left
                    sleep 300
                }
                if (autoCloseChat) {
                    sleep 300
                    Send {/}    ; chat shortcut key
                    sleep 300
                    MouseMove, 149, 40, 3 ; Chat button
                    sleep 300
                    MouseClick, Left
                    sleep 300
                }
                ...
                Send, {Enter}       ;  |
                sleep 2600          ; <-
                MouseMove, 47, 467, 3 ; Auras Collection
                sleep 220
                Click, Left
                sleep 220
                MouseMove, 382, 126, 3 ; Exit Auras Collection Button
                sleep 220
                Click, Left
                sleep 220
                Click, WheelUp 80
                ...
                if (pathingMode = "Non Vip Pathing") {
                    ; Non VIP Pathing
                    Send, {%keyW% Down}     ; <--- Navigate to Fish
                    Send, {%keyA% Down}     ;   |
                    sleep 5190              ;   |
                    Send, {%keyW% Up}       ;   |
                    sleep 800               ;   |
                    ...
                    Send {e Up}             ;   |
                    sleep 300               ; <--
                    MouseMove, 956, 803, 3 ; Fish Merchant Dialog Box
                    sleep 50
                    MouseClick, Left
                    sleep 50
                    MouseClick, Left
                    sleep 200
                    MouseMove, 956, 938, 3 ; Fish Merchant Dialog Box => Sell Fish Button
                    sleep 200
                    MouseClick, Left
                    sleep 800
                    loopCount := 0
                    ...
                    while (loopCount < fishingLoopCount) {
                        MouseMove, 828, 404, 3 ; Fish Inventory Slot one
                        sleep 200
                        MouseClick, Left
                        sleep 200
                        if (sellAllToggle) {
                            MouseMove, 680, 804, 3 ; Sell All button
                        } else {
                            MouseMove, 512, 804, 3 ; Sell One button
                        }
                        sleep 200
                        MouseClick, Left
                        sleep 300
                        MouseMove, 801, 626, 3 ; Sell Comfirmation Button
                        sleep 200
                        MouseClick, Left
                        sleep 1000
                        loopCount++
                    ...
                    MouseMove, 1458, 266, 3 ; Close Fish Merchant Menu
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    Send, {%keyA% Down}     ;<--- Navigate to fishing Spot near tree
                    sleep 1400              ;  |
                    Send, {%keyA% Up}       ;  |
                    sleep 75                ;  |
                    Send, {%keyW% Down}     ;  |
                    ...
                    ; Abyssal Pathing
                    MouseMove, 30, 406, 3   ; Auras Inventory
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    MouseMove, 947, 335, 3 ; Frist Aura Tab (NON SPECIAL TAB)
                    sleep 200
                    MouseClick, Left
                    sleep 100
                    MouseMove, 1102, 367, 3 ; Search Box
                    sleep 100
                    MouseClick, Left
                    sleep 100
                    ClipBoard := "Abyssal Hunter"
                    ...
                    Send, ^v
                    sleep 200
                    MouseMove, 819, 434, 3 ; Aura Slot One
                    sleep 200
                    Click, WheelUp 100
                    sleep 200
                    MouseClick, Left
                    ...
                    PixelSearch, px, py, 576, 626, 666, 645, 0xfc7f98, 3, Fast RGB
                    if (ErrorLevel != 0) {
                        MouseMove, 623, 634, 3 ;Unequip/Equipe Button
                        sleep 200
                        MouseClick, Left
                    }

                    sleep 200
                    MouseMove, 1412, 296, 3 ; Close Gui Button
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    ...
                    MouseMove, 981, 805, 3 ; Fish Merchant Dialog Box
                    sleep 50
                    MouseClick, Left
                    sleep 50
                    MouseClick, Left
                    sleep 200
                    MouseMove, 967, 948, 3 ; Fish Merchant Dialog Box => Sell Fish Button
                    sleep 200
                    MouseClick, Left
                    sleep 800
                    loopCount := 0
                    ...
                    while (loopCount < fishingLoopCount) {
                        MouseMove, 838, 413, 3 ; Fish Inventory Slot one
                        sleep 200
                        MouseClick, Left
                        sleep 200
                        if (sellAllToggle) {
                            MouseMove, 678, 810, 3 ; Sell All Button
                        } else {
                            MouseMove, 525, 809, 3 ; Sell One Button
                        }
                        sleep 200
                        MouseClick, Left
                        sleep 300
                    ...
                    MouseMove, 1469, 271, 3 ; Close Fish Merchant Menu
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    Send, {%keyA% Down}        ; <--- Navigate to fishing Spot near tree
            ...
            MouseMove, 862, 843, 3 ; Click Fish button on minigame popup
            Sleep 300
            MouseClick, Left
            sleep 300
            barColor := 0
            ...
            Loop {
                ;Look for white pixel of fishing indicator at bottom right of window
                PixelGetColor, color, 1176, 836, RGB
                if (color = 0xFFFFFF) {
                    MouseMove, 950, 880, 3 ; Move mouse below box
                ...
                if (A_TickCount - globalFailsafeTimer > (autoRejoinFailsafeTime * 1000) && privateServerLink != "") {
                    ...
                    if (checkColor != 0xFFFFFF) {
                        ...
                        Loop {
                            ErrorLevel := 0
                            ;look for the main menu button color
                            PixelSearch, px, py, 205, 1019, 325, 978, 0x82ff95, 5, Fast RGB
                            if (ErrorLevel = 0) {
                                sleep 1000
                                MouseMove, 267, 1000, 3 ; Start button on Main menu
                                sleep 350
                                MouseClick, Left
                                break
                            }
                ...
                if (A_TickCount - startWhitePixelSearch > (fishingFailsafeTime * 1000) && !fishingFailsafeRan) {
                    MouseMove, 1268, 941, 3 ; Failsafe Click one?
                    sleep 300
                    MouseClick, Left
                    sleep 300
                    MouseMove, 1167, 476, 3  ; Failsafe Click two?
                    sleep 300
                    MouseClick, Left
                    sleep 300
                    MouseMove, 1113, 342, 3  ; Failsafe Click three?
                    sleep 300
                    MouseClick, left
                    sleep 300
                    MouseMove, 851, 832, 3  ; Most likely the Fish Minigame Button
                    sleep 300
                    MouseClick, Left
            ...
            sleep 300
            MouseMove, 1113, 342, 3 ; Close Post Fishing Menu
            Sleep 700
            /*
            Loop {
    ...
    if (toggle) {
        ...
        Loop {
            ...
            if (loopCount > maxLoopCount || restartPathing) {
                ...
                if (autoUnequip) {
                    MouseMove, 41, 538, 3 ; Auras Inventory
                    sleep 300
                    Click, Left
                    sleep 300
                    MouseMove, 1089, 575, 3 ; Slot One
                    sleep 300
                    Click, Left
                    sleep 300
                    MouseMove, 835, 845, 3 ; Unequip/Equip button
                    sleep 300
                    Click, Left
                    sleep 1200
                    Click, Left
                    sleep 300
                    MouseMove, 1882, 395, 3 ; Close Gui Button
                    sleep 300
                    Click, Left
                    sleep 300
                }
                if (autoCloseChat) {
                    ...
                    Send {/}    ; chat shortcut key
                    sleep 300
                    MouseMove, 151, 38, 3 ; Chat button
                    sleep 300
                    MouseClick, Left
                    sleep 300
                }
                ...
                Send, {Enter}   ;  |
                sleep 2600      ; <-
                MouseMove, 52, 621, 3 ; Auras Collection
                sleep 220
                Click, Left
                sleep 220
                MouseMove, 525, 158, 3 ; Exit Auras Collection Button
                sleep 220
                Click, Left
                sleep 220
                Click, WheelUp 80
                ...
                if (pathingMode = "Non Vip Pathing") {
                    ...
                    Send {e Up}             ;   |
                    sleep 300               ; <--
                    MouseMove, 1308, 1073, 3 ; Fish Merchant Dialog Box
                    sleep 50
                    MouseClick, Left
                    sleep 50
                    MouseClick, Left
                    sleep 200
                    MouseMove, 1289, 1264, 3 ; Fish Merchant Dialog Box => Sell Fish Button
                    sleep 200
                    MouseClick, Left
                    sleep 800
                    loopCount := 0
                    ...
                    while (loopCount < fishingLoopCount) {
                        MouseMove, 1117, 550, 3 ; Fish Inventory Slot one
                        sleep 200
                        MouseClick, Left
                        sleep 200
                        if (sellAllToggle) {
                            MouseMove, 904, 1080, 3 ; Sell All button
                        } else {
                            MouseMove, 700, 1078, 3 ; Sell One button
                        }
                        sleep 200
                        MouseClick, Left
                        sleep 300
                        MouseMove, 1002, 831, 3 ; Sell Comfirmation Button
                        sleep 200
                        MouseClick, Left
                        sleep 1000
                        loopCount++
                    ...
                    MouseMove, 1958, 361, 3 ; Close Fish Merchant Menu
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    Send, {%keyA% Down}     ;<--- Navigate to fishing Spot near tree
                    ...
                    ; Abyssal Pathing
                    MouseMove, 40, 541, 3   ; Auras Inventory
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    MouseMove, 1262, 447, 3 ; Frist Aura Tab (NON SPECIAL TAB)
                    sleep 200
                    MouseClick, Left
                    sleep 100
                    MouseMove, 1469, 489, 3 ; Search Box
                    sleep 100
                    MouseClick, Left
                    sleep 100
                    ClipBoard := "Abyssal Hunter"
                    ...
                    Send, ^v
                    sleep 200
                    MouseMove, 1092, 579, 3 ; Aura Slot One
                    sleep 200
                    Click, WheelUp 100
                    sleep 200
                    MouseClick, Left
                    ...
                    PixelSearch, px, py, 768, 835, 888, 860, 0xfc7f98, 3, Fast RGB
                    if (ErrorLevel != 0) {
                        MouseMove, 830, 845, 3 ;Unequip/Equipe Button
                        sleep 200
                        MouseClick, Left
                    }

                    sleep 200
                    MouseMove, 1883, 395, 3 ; Close Gui Button
                    sleep 200
                    MouseClick, Left
                    sleep 200
            ...
            ; Fishing Minigame
            MouseMove, 1161, 1124, 3 ; Click Fish button on minigame popup
            Sleep 30
            MouseClick, Left
            sleep 300
            barColor := 0
            ...
            Loop {
                ...
                if (color = 0xFFFFFF) {
                    MouseMove, 1263, 1177, 3 ; Move mouse below box
                    ; Get randomized bar color
                    Sleep 50
            ...
            if (A_TickCount - globalFailsafeTimer > (autoRejoinFailsafeTime * 1000) && privateServerLink != "") {
                ...
                if (checkColor != 0xFFFFFF) {
                    ...
                    Loop {
                        ...
                        if (ErrorLevel = 0) {
                            sleep 1000
                            MouseMove, 347, 1329, 3 ; Start button on Main menu
                            sleep 350
                            MouseClick, Left
                            break
                        }
            ...
            ; Fishing Failsafe
            if (A_TickCount - startWhitePixelSearch > (fishingFailsafeTime * 1000) && !fishingFailsafeRan) {
                MouseMove, 1690, 1224, 3 ; Failsafe Click one?
                sleep 300
                MouseClick, Left
                sleep 300
                MouseMove, 1523, 649, 3  ; Failsafe Click two?
                sleep 300
                MouseClick, Left
                sleep 300
                MouseMove, 1457, 491, 3  ; Failsafe Click three?
                sleep 300
                MouseClick, left
                sleep 300
                MouseMove, 1163, 1126, 3  ; Most likely the Fish Minigame Button
                sleep 300
                MouseClick, Left
                fishingFailsafeRan := true
                if (failsafeWebhook) {
        ...
        sleep 300
        MouseMove, 1457, 491, 3 ; Close Post Fishing Menu
        sleep 700
        /*
        Loop {
    ...
    if (toggle) {
        ...
        Loop {
            ...
            if (loopCount > maxLoopCount || restartPathing) {
                ...
                if (autoUnequip) {
                    MouseMove, 38, 292, 3 ; Auras Inventory
                    sleep 300
                    Click, Left
                    sleep 300
                    MouseMove, 594, 314, 3 ; Slot One
                    sleep 300
                    Click, Left
                    sleep 300
                    MouseMove, 458, 457, 3 ; Unequip/Equip button
                    sleep 300
                    Click, Left
                    sleep 1200
                    Click, Left
                    sleep 300
                    MouseMove, 1016, 218, 3 ; Close Gui Button
                    sleep 300
                    Click, Left
                    sleep 300
                }
                if (autoCloseChat) {
                    ...
                    Send {/}                ; chat shortcut key
                    sleep 300
                    MouseMove, 151, 42, 3 ; Chat button
                    sleep 300
                    MouseClick, Left
                    sleep 300
                }
                ...
                Send, {Enter}    ;  |
                sleep 2600       ; <-
                MouseMove, 26, 325, 3 ; Auras Collection
                sleep 220
                Click, Left
                sleep 220
                MouseMove, 273, 106, 3 ; Exit Auras Collection Button
                sleep 220
                Click, Left
                sleep 220
                Click, WheelUp 80
                ...
                if (pathingMode = "Non Vip Pathing") {
                    ...
                    Send {e Up}             ;   |
                    sleep 300               ; <--
                    MouseMove, 682, 563, 3 ; Fish Merchant Dialog Box
                    sleep 50
                    MouseClick, Left
                    sleep 50
                    MouseClick, Left
                    sleep 200
                    MouseMove, 682, 667, 3 ; Fish Merchant Dialog Box => Sell Fish Button
                    sleep 200
                    MouseClick, Left
                    sleep 800
                    loopCount := 0
                    ...
                    while (loopCount < fishingLoopCount) {
                        MouseMove, 586, 287, 3 ; Fish Inventory Slot one
                        sleep 200
                        MouseClick, Left
                        sleep 200
                        if (sellAllToggle) {
                            MouseMove, 486, 570, 3 ; Sell All button
                        } else {
                            MouseMove, 365, 570, 3 ; Sell One button
                        }
                        sleep 200
                        MouseClick, Left
                        sleep 300
                        MouseMove, 573, 447, 3 ; Sell Comfirmation Button
                        sleep 200
                        MouseClick, Left
                        sleep 1000
                        loopCount++
                    ...
                    MouseMove, 1050, 197, 3 ; Close Fish Merchant Menu
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    Send, {%keyA% Down}     ;<--- Navigate to fishing Spot near tree
                    ...
                    ; Abyssal Pathing
                    MouseMove, 21, 289, 3   ; Auras Inventory
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    MouseMove, 675, 239, 3 ; Frist Aura Tab (NON SPECIAL TAB)
                    sleep 200
                    MouseClick, Left
                    sleep 100
                    MouseMove, 786, 261, 3 ; Search Box
                    sleep 100
                    MouseClick, Left
                    sleep 100
                    ClipBoard := "Abyssal Hunter"
                    ...
                    Send, ^v
                    sleep 200
                    MouseMove, 584, 310, 3 ; Aura Slot One
                    sleep 200
                    Click, WheelUp 100
                    sleep 200
                    MouseClick, Left
                    ...
                    PixelSearch, px, py, 411, 446, 475, 460, 0xed7389, 3, Fast RGB
                    if (ErrorLevel != 0) {
                        MouseMove, 444, 452, 3 ;Unequip/Equipe Button
                        sleep 200
                        MouseClick, Left
                    }

                    sleep 200
                    MouseMove, 1007, 211, 3 ; Close Gui Button
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    ...
                    MouseMove, 699, 574, 3 ; Fish Merchant Dialog Box
                    sleep 50
                    MouseClick, Left
                    sleep 50
                    MouseClick, Left
                    sleep 200
                    MouseMove, 689, 676, 3 ; Fish Merchant Dialog Box => Sell Fish Button
                    sleep 200
                    MouseClick, Left
                    sleep 800
                    loopCount := 0
                    ...
                    while (loopCount < fishingLoopCount) {
                        ...
                        sleep 200
                        if (sellAllToggle) {
                            MouseMove, 484, 577, 3 ; Sell All Button
                        } else {
                            MouseMove, 374, 576, 3 ; Sell One Button
                        }
                        sleep 200
                        MouseClick, Left
                        sleep 300
                    ...
                    MouseMove, 1047, 193, 3 ; Close Fish Merchant Menu
                    sleep 200
                    MouseClick, Left
                    sleep 200
                    Send, {%keyA% Down}        ; <--- Navigate to fishing Spot near tree
            ...
            ; Fishing Minigame
            MouseMove, 603, 597, 3 ; Click Fish button on minigame popup
            Sleep 300
            MouseClick, Left
            sleep 300
            barColor := 0
            ...
            Loop {
                ...
                if (ErrorLevel = 0) {
                    MouseMove, 676, 638, 3 ; Move mouse below box
                ...
                if (A_TickCount - globalFailsafeTimer > (autoRejoinFailsafeTime * 1000) && privateServerLink != "") {
                    ...
                    if (checkColor != 0xFFFFFF) {
                        ...
                        Loop {
                            ...
                            if (ErrorLevel = 0) {
                                sleep 1000
                                MouseMove, 200, 715, 3 ; Start button on Main menu
                                sleep 350
                                MouseClick, Left
                                break
                            }
                ...
                ; Fishing Failsafe
                if (A_TickCount - startWhitePixelSearch > (fishingFailsafeTime * 1000) && !fishingFailsafeRan) {
                    MouseMove, 902, 668, 3 ; Failsafe Click one?
                    sleep 300
                    MouseClick, Left
                    sleep 300
                    MouseMove, 858, 331, 3  ; Failsafe Click two?
                    sleep 300
                    MouseClick, Left
                    sleep 300
                    MouseMove, 817, 210, 3  ; Failsafe Click three?
                    sleep 300
                    MouseClick, left
                    sleep 300
                    MouseMove, 588, 588, 3  ; Most likely the Fish Minigame Button
                    sleep 300
                    MouseClick, Left
                    fishingFailsafeRan := true
                    if (failsafeWebhook) {
...
CloseScript:
    ...
    try SendWebhook(":red_circle: Macro Stopped.", "14495300")
         ; Just a quick note for anyone looking to this code
         ; as a source of learning, calling return after 
 ExitApp ; ExitApp is pretty much pointless - Nadir :P 
return   
         
; saves the current Resolution Selection to the ini/settings file
SelectRes:
    Gui, Submit, nohide
    res := Resolution
    IniWrite, %res%, %iniFilePath%, "Macro", "resolution"
...
;handle clicking on things in ui
; ie the devs names and links
;     donate or support links
;     ect----
;<==
 ; saves the current pathing mode to the ini/settings file
 SelectPathing:
    Gui, Submit, nohide
 ...
 ; if the dev1_name is specifically "cresqnt" we open their url
 Dev1NameClick:
    if (dev1_name = "cresqnt") {
 ...
 ; using the dev1_name we open their url
 Dev1LinkClick:
 ...
 ; if the dev2_name is specifically "cresqnt" we open their url
 Dev2NameClick:
    if (dev2_name = "cresqnt") {
        Run, https://cresqnt.com
    }
 ...
 ; using the dev2_name we open their url
 Dev2LinkClick:
 ...
 ; if the dev3_name is specifically "cresqnt" we open their url
 Dev3NameClick:
    if (dev3_name = "cresqnt") {
        Run, https://cresqnt.com
    }
 ...
 ; using the dev3_name we open their url
 Dev3LinkClick:
 ...
 return
                                                ; v== possibly a bug
 ; clicking the <donation> text, Robux Image or Discord Image opens the fishsols donations page
 DonateClick:
    Run, https://www.roblox.com/games/106268429577845/fishSol-Donations#!/store
 return
 
 ; clicking the <need help> text opens the fishsols donations page
 NeedHelpClick:
    Run, https://discord.gg/nPvA54ShTm
 return
 
 ; opens the folder containing the plugins
 OpenPluginsFolder:
            ;| this is the directory of this script
            ;|           | the sub directory plugins
            ;v           v
