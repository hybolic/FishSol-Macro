...
if not CONFIG_ONLY_HOTKEYS
{
	...
	REGEX_DEPATCH := "([ ]{8}MouseMove..[\d]{3,4}..[\d]{3,4}..\d)\n[\w ,]*\n[\w ,]*\n(([ ]+Clipboard....[^A][\w ]+.)\n(?:\s+sleep.300)\n(\s+Send..\^v)\n(?:\s+sleep.300)\n(\s+MouseMove..[\d]{3,4}?..[\d]{3,4}?..3)\n(?:\s+sleep.300)\n(\s+MouseClick..Left)\n(\s+sleep.300)\n+)(?(?=\n;|;)(?:\s*?;{25,35}[\w\s]*?;{25,35}))\n(([ ]{4}([ ]{4}))Loop(?:\s|\s\d\s){)(?(?!\n[ ]{12}PixelSearch)(?:\n[\w, \^]*){7,8}|(?:))((?:(\n.{12}).{21}[\d]{3,4}?..[\d]{3,4}?..[\d]{3,4}?..[\d]{3,4}?..0x[0-9A-Fa-f]{6}..\d.{10}\g12if.\(!ErrorLevel\))[\w\W]{310,320}})(?(?=\n;)(?:\n*;*[\w\s]*;*)\n|)(?=\s*MouseMove)"
	REGEX_BIOME_FIX := "(([ ]{8}MouseMove..[\d]+..[\d]+..\d)\n[\w ,]*\n[\w ,]*\n([ ]+Clipboard....[^A][\w ]+.)\n(?:\s+sleep.300)\n(?:;*[\w]*;*)(\s+Send..\^v)\n(?:\s+sleep.300)\n(\s+MouseMove..[\d]+?..[\d]+?..3)\n(?:\s+sleep.300)\n(\s+MouseClick..Left)\n(\s+sleep.300)\n+)(?:\n*?.*?[\w\s]?.*\n*?)(([ ]{4}([ ]{4}))Loop(?:\s|\s\d\s){)(?:[\w\W]*?(?=\n.{12}PixelSearch))((?:\n[ ]{12}.{21}[\d]+?..[\d]+?..[\d]+?..[\d]+?..0x[0-9A-Fa-f]{6}..\d.{10}\n[ ]{12}if.\(!ErrorLevel\))[\w\W]{310,320}?})"
	BIOME_CODE := "$1;;;;;;;;;;;;;;;;;;;;;;;;;;;;NADIR BIOME PATCHES;;;;;;;;;;;;;;;;;;;;;;;;;;;"
				. "`n$8" . "`n${10}$2" . "`n${10}$7" . "`n${10}$6" . "`n${10}$3" . "`n${10}$7"
				. "`n${10}$4" . "`n${10}$7" . "`n${10}$5" . "`n${10}$7" . "`n${10}$6" . "`n${10}$7${11}"
				. "`n;;;;;;;;;;;;;;;;;;;;;;;;;;;;END OF BIOME PATCHES;;;;;;;;;;;;;;;;;;;;;;;;;;;"
...
if not CONFIG_ONLY_HOTKEYS
{
	file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_VARIBLES , VARIBLES_CODE, "Patch_TOF")
	if(PUSH_GOLEM_CODE)
	{
	...
	temp_obj := getRegexObject(file_contents1 , REGEX_BIOME_FIX, REGGIBIOME)
	file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_BIOME_FIX, BIOME_CODE, "Patch_BiomeFixer")
	file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_NUDGE_CODE_LOCATION, NUDGE_CODE, "Patch_Fishing_Nudger")
	...
	file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_GUI_tabList, GUI_tabList_MODIFICATION, "Patch_Tablist") 
	file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_PATCH_TAB, GUI_PATCHES_TAB,"Patch_Gui") 
	file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_INI, INI_MODIFICATION, "Patch_INI")
	file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_END_OF_FILE, END_OF_FILE, "Patch_EOF")
}
file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_GUI_HOTKEYS, GUIHOTKEYS_CODE, "Patch_HotKeys")
file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_START_HOTKEY, KeyStart_WAITING, "Patch_KeyStart")
file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_PAUSE_HOTKEY, KeyPause_WAITING, "Patch_KeyPause")
file_contents1 := RegExReplaceAndStore(file_contents1 , REGEX_STOP_HOTKEY, KeyStop_WAITING, "Patch_Stop")
...
RegExReplaceAndStore(file_cont, Regex, Replace, Patch_Name)
{
	ret := RegExReplace(file_cont,Regex, Replace)
	reg := StrReplace(Regex, "`n", "\n")
	rep := StrReplace(Replace, "`n", "\n")
	FileAppend, % Patch_Name "_REGEX: " reg "`n", % A_ScriptDir "\Strings.txt"
	FileAppend, % Patch_Name "_PATCH: " rep "`n", % A_ScriptDir "\Strings.txt"
	return ret
}
